...
class DebianPackageTests extends munit.FunSuite with PackageHelper {
  ...
  test("should create DEBIAN directory ") {
    ...
    val expectedEchoLauncherPath = debianDirectoryPath / "usr" / "share" / "scala" / packageName
  ...
  test("should generate dep package") {
    ...
    val payloadFiles = os.proc("dpkg", "--contents", expectedDepPath).call().out.text().trim
    val expectedScriptPath = os.RelPath("usr") / "bin" / packageName
    val expectedEchoLauncherPath = os.RelPath("usr") / "share" / "scala" / packageName
